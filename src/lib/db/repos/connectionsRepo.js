...
// Internal reorder — must be called INSIDE a transaction (handle = tx)
async function reorderInTx(h, providerId) {
  const rows = await h.all(`SELECT * FROM providerConnections WHERE provider = ?`, [providerId]);
  const list = rows.map(rowToConn);
  list.sort((a, b) => {
  ...
  for (let i = 0; i < list.length; i++) {
    await h.run(`UPDATE providerConnections SET priority = ? WHERE id = ?`, [i + 1, list[i].id]);
  }
}
...
export async function createProviderConnection(data) {
  ...
  await db.transaction(async (tx) => {
    const all = (await tx.all(`SELECT * FROM providerConnections WHERE provider = ?`, [data.provider])).map(rowToConn);
